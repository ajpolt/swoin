...
public class ResolvableDependency<T>: Dependency {
    ...
    public func bind<NewType>(_ newType: NewType.Type, _ converter: @escaping (T) -> NewType) -> BoundDependency<T, NewType> {
    ...
    public func then(_ closure: @escaping (T) -> ()) -> ResolvableDependency<T> {
