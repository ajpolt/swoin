...
public class Module: SwoinParameter {
    ...
    func register<T>(_ type: T.Type = T.self, named name: String? = nil, cacheType: CacheType, factory: @escaping () -> T) {
    ...
    func bind<ExistingType, AdditionalType>(additionalType: AdditionalType.Type, to existingType: ExistingType.Type, named name: String?, converter: @escaping (ExistingType) -> AdditionalType = { existing in existing as! AdditionalType }) {
    ...
    public func resolve<T>(_ type: T.Type = T.self, named name: String? = nil) -> T {
...
// Function builders will fail with 0 or 1 arguments
// It looks like there is a fix and it should be available in Swift 5.3
// Added mostly for completeness. Why you'd need this, I don't actually know.
public func EmptyModule() -> Module {
    let module = Module()
    return module
}
